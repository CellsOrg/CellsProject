# 02_USER_FLOW — 박세일즈의 하루 (Before/After)
...
## 1. Before — Customer 360 도입 전

박세일즈는 중구·종로구 약 **1,300개 매장**을 담당한다. Sales와 CS 데이터가 나뉜 시스템에 있어서,
어떤 매장이 Tongss Step 도입 가능성이 높은지 알 방법이 없다.

```mermaid
flowchart TD
    A["09:00 출근, Salesforce 로그인"] --> B["1,300개 담당 매장 리스트를 본다<br/>— 우선순위 정보 없음"]
...
**문제의 핵심:** CS 시스템에 "이 매장 POS 오조작으로 문의가 반복된다"는 신호가 이미 쌓여 있어도,
그 정보가 Sales(Salesforce) 쪽으로 넘어오지 않는다. 박세일즈는 이대표(함부기)의 매장이
Step 도입 신호를 보내고 있다는 사실을 알 방법이 없다.

---

## 2. After — Customer 360 도입 후
...
**바뀐 것:** 1,300개 전체를 보는 게 아니라, CS 신호로 좁혀진 소수(10개)만 본다.
그리고 그 10개는 항상 "왜 이 매장인가"라는 근거를 달고 온다 — `01_PERSONAS.md`의
"이 사람을 위해 지킬 것" 참조.

---

## 3. 이대표(함부기) 쪽 흐름 — CS Case에서 Lead까지

박세일즈의 "After"가 가능한 이유는, 그 이전에 CS 쪽에서 아래 흐름이 먼저 돌아가고 있기 때문이다.
...
- Before/After는 항상 박세일즈 1인 기준이다. 다른 영업 담당자 시나리오를 새로 만들지 않는다.
- 함부기 쪽 흐름은 "Case 등록 → 반복 조건 → Recommendation 자동 생성"까지만 다룬다(Opportunity가 아니다
  — §3 참고). Recommendation 이후 실제 영업 대화 내용은 `01_PERSONAS.md`의 "성공의 정의"를 넘어서는
  디테일이므로 여기서 다루지 않는다.
...
- [`00_PRODUCT_GUIDE.md`](./00_PRODUCT_GUIDE.md) — 이 하루 흐름이 왜 중요한지(한 줄 목표)
