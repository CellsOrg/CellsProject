# 01_DOMAIN_MODEL_2.md — Salesforce Customer 360 Business Domain Model

> 이전 버전(`01_DOMAIN_MODEL.md`)은 **Story**(00_STORY.md — 키티, 쿠로미, 마이멜로디 등 인물의 이야기) 관점에서
> "이 세계에 어떤 명사가 존재하는가"를 정의한 문서다.
> 이 문서는 같은 질문을 **Workflow**(`02_WORKFLOW.md` — 마케팅/티켓/멤버십/굿즈/고객지원/스폰서십 6개 팀의 실제 업무
> 흐름)를 기준으로 다시 검증한다. Story는 "왜 이 데이터가 의미 있는가"를 보여주고, Workflow는 "실제 업무에서
> 어떤 명사가 반드시 필요한가"를 보여준다 — 이 둘은 서로 다른 렌즈이며, 둘 다 맞을 수 있다.
>
> 기존 문서의 구조(Domain Overview → Entity 목록 → Entity 간 관계 → Salesforce Mapping)와 철학(Story보다
> Business, Salesforce보다 Domain, 명사 중심, Field는 아직 안 만듦)은 그대로 유지한다. 달라진 점은 **왜
> 이 Entity가 필요한지에 대한 근거가 Story의 서사가 아니라 Workflow의 업무 단계**라는 것이다.
...
기존 문서는 Fan / Operations / Partnership 3개 Domain을 제시했다. `02_WORKFLOW.md`의 6개 팀 업무를
그대로 이 3개 Domain에 밀어넣어 보면, **두 팀의 업무가 어디에도 깔끔하게 들어가지 않는다.**
...
`02_WORKFLOW.md`의 6개 팀 Workflow를 각 단계별로 훑으며 등장하는 명사를 뽑는다. 화살표(→)는 업무
순서이지, Entity 간의 데이터 관계가 아니다(관계는 §5에서 별도로 정리한다).

### 2.1 마케팅 Workflow

```
시장·팬 분석 → 팬 세그먼트(Fan Segment) → 캠페인(Campaign) 기획
→ 세분화 기준(관람 이력, 구매 이력, 관심도) → 수신 동의(Marketing Consent) 확인 → 발송 대상 확정
→ 콘텐츠·쿠폰·프로모션 발송(Benefit) → 반응(열람·클릭·참여·구매) → 캠페인 성과(Campaign Performance)
```
...
```
판매 준비(Ticket Policy, Seat, Game) → 자격 확인(Eligibility Rule) → 예매(Ticket Purchase)
→ 티켓 발급(Ticket) → 선물·양도(Ticket Transfer) → 입장(Gate, Admission) → 문제 처리(Refund/Cancellation)
→ 관람 분석(Attendance Record)
```
...
```
팬 분석(Fan Activity Pattern) → 멤버십 상품 기획(Membership Tier) → 가입(Membership Enrollment)
→ 활성화·발급(Membership Card) → 혜택 운영(Benefit, Benefit Redemption) → 이용 분석 → 갱신 관리(Renewal)
```
...
```
수요 분석(Player, Product Category) → 상품 기획(Goods) → 출시 대상 선정 → 개인화 추천(Recommendation)
→ 혜택 적용(Benefit) → 주문 처리(Goods Purchase) → 배송 관리(Shipment) → 교환·반품(Return)
→ 재구매 분석
```
...
```
문의 접수(Channel, Inquiry) → 본인 확인(Fan) → 문의 파악 → 유형별 처리(Ticket/Membership/Goods 문의로 분류)
→ 해결·이관 → 문의 종료 → 재발 방지
```
...
```
스폰서십 상품 기획(Sponsorship Package) → 스폰서 발굴(Sponsor, Partner Contact) → 제안(Proposal)
→ 계약(Sponsor Contract) → 캠페인 설계(Campaign) → 실행(Benefit) → 성과 측정(Sponsor Performance)
→ 재계약(Renewal)
```
...
| Membership Card | 멤버십 (활성화·발급) | 디지털/실물 카드 발급이 별도 업무 단계로 명시되어 있다. (다만 §3.3에서 Entity로 만들지, 필드로 둘지는 다시 논의한다.) |
...
대부분의 기존 Entity(Fan, Player, Staff, Hanwha Eagles, Sponsor, Ticket, Season Pass, Membership,
...
| Licensor | **별도 Entity에서 제외 → `Partner`로 흡수** | `00_STORY.md`(산리오 사례)에는 등장하지만, `02_WORKFLOW.md`의 스폰서십 Workflow는 스폰서와 라이선스사를 구분하지 않고 "스폰서 발굴→제안→계약"이라는 동일한 절차로 처리한다. 실제 업무 프로세스가 다르지 않다면, 별도 Entity보다 `Partner`의 유형(스폰서/라이선스/제휴 등) 정도로 남기는 것이 Workflow에 더 부합한다. Story가 보여준 "산리오는 다른 종류의 파트너"라는 통찰은 사라지지 않는다 — Entity 레벨이 아니라 값(분류) 레벨로 내려갈 뿐이다. |
...
| Entity | 설명 | Domain |
|---|---|---|
| Hanwha Eagles | 구단 본체 | 전 Domain 공통 |
...
```
[Operations 축 — 구단이 파는 것]

Ticket Policy / Eligibility Rule
        ↓ (판매 조건을 정의)
Game ─── Ballpark ─── Section ─── Seat ─── Gate
        ↓
Ticket / Season Pass
        ↓
Ticket Purchase ──→ Ticket Transfer (구매자 ≠ 입장자일 때)
        ↓
Admission (게이트 통과) ──→ Attendance Record (누적 집계)


Membership Tier
        ↓
Membership ─── Membership Enrollment ─── Membership Card
        ↓
Benefit ─── Benefit Redemption
        ↑
Goods ─── Goods Purchase ─── Shipment
                  ↓
                Return
```
...
Fan ─── Attendance Record / Engagement Signal / Fan Activity Pattern
   ↓
Fan Segment ──→ Campaign ──→ Notification (Marketing Consent 확인 후 발송)
                  ↓
             Campaign Performance

Fan ──→ Recommendation (개인화 추천)
Fan ──→ Inquiry (Ticket / Membership / Goods Purchase 등을 참조하여 문의 처리)
```
...
Partner Contact
   ↓
Sponsorship Package ──→ Proposal ──→ Sponsor Contract ──→ Renewal
                                          ↓
                                     Campaign ─── Collaboration Item
                                          ↓
                                     Partner Store
                                          ↓
...
한화이글스 CRM은 Fan(개인)이 압도적으로 많고, 회사 소속과 무관하게 직접 티켓을 사고 멤버십에
...
`02_WORKFLOW.md`의 티켓팀 Workflow는 "선물·양도"를 "문제 처리(양도 오류)"와 별도 단계로 명시하고
있다 — 즉 양도가 CS 이슈로 이어질 수 있다는 뜻이다. 이 빈도가 높다고 예상되면 선택 2가, Demo 범위
안에서는 단순 표시로 충분하다면 선택 1이 맞다.
...
`00_STORY.md`의 쿠로미 사례("3년간 42경기 → 올해 0경기"를 시스템이 감지해서 메시지를 자동 발송)를
보면, 이 판단이 **자동화(Notification 발송)의 트리거**로 쓰인다 — 이건 선택 1(저장) 쪽에 힘을
싣는 근거다. 반대로 단순히 "관리자가 대시보드에서 훑어보는" 용도라면 선택 2로 충분하다. Renewal도
같은 기준으로 판단하면 된다 — 갱신 임박을 이유로 자동 알림을 보낼 계획이면 저장, 관리자가 리포트로
확인만 하면 된다면 비저장.
...
- 장점: Partner(성심당 같은 협업사)와 Partner Store(그 협업사의 매장)를 Account-Account 관계
...
Partner Store가 등장하는 이유(B2B 콜라보 — "빵을 산 사람이 경기장에도 왔는가")를 생각하면, 핵심은
"이 매장이 어느 Partner 소속인가"라는 관계다. 관계가 핵심이면 선택 1이, 매장 자체의 운영 데이터가
더 중요해지면 선택 2가 맞다.
...
다음 단계(`04_DATA_MODEL.md`)로 넘어가기 전에, §6의 각 "우리 프로젝트에서 고민해야 하는 점"에
대한 팀의 판단을 먼저 확정하는 것을 권한다 — 특히 §6.5(Order vs Opportunity)와 §6.3(Product2 여부)은
서로 연결되어 있어 함께 결정하는 것이 좋다.
